...
from werkzeug.contrib.atom import AtomFeed # Works because you downgraded Werkzeug
...
@app.route('/')
def home():
    # Attempt to get Advertisements
    response = requests.get(settings.API_URL + '/getAdvertisements')
    
    # FIX: Safety check for JSONDecodeError
    if response.status_code != 200:
        return f"<h1>Backend Error</h1><p>Status: {response.status_code}</p><p>Check if your Azure Function is running.</p>"

    try:
        ads = response.json()
    except Exception as e:
        return f"<h1>Data Error</h1><p>The backend returned invalid data. Raw response: {response.text}</p>"

    # Attempt to get Posts
    response2 = requests.get(settings.API_URL + '/getPosts')
    posts = response2.json() if response2.status_code == 200 else []
